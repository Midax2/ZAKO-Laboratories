...
void DLXJITArm7::writeEor(Condition cond, bool updateFlags, Register dest, Register src1, Register src2) {
	DataProcessingRegisterInstruction instr;
	instr.cond = cond;
	instr.b27 = false;
	instr.b26 = false;
...
void DLXJITArm7::writeMul(Condition cond, bool updateFlags, Register dest, Register src1, Register src2) {
	...
	instr.b24 = 0;
	instr.op = 0;
	instr.detail1 = dest << 8 | 0xB << 4 | src1;
	instr.b7 = true;
	instr.b6 = false;
	instr.b5 = false;
	instr.b4 = true;
...
Register DLXJITArm7::loadDLXRegister(int no,int argumentNumber) {
    ...
    {
        Register ret = argumentNumber == 0 ? FIRST_ARGUMENT_CACHE_REGISTER : SECOND_ARGUMENT_CACHE_REGISTER;
        if(no == 0)
        {
...
void DLXJITArm7::compileDLXInstruction(const DLXJITCodLine& line) {
	...
	else if (line.textInstruction->opcode() == "XOR")
	{
		...
		if (destination > 0)
		{
			...
			Register src2 = loadDLXRegister(getDLXRegisterNumber(instr->reg(1)), 1);
			Register dst = getRegisterForTargetDLXRegister(destination);
			writeEor(AL, false, dst, src1, src2);
			storeTargetDLXRegister(destination);
		}
	}
	else if (line.textInstruction->opcode() == "MUL")
	{
		...
		if (destination > 0)
		{
			...
			Register src2 = loadDLXRegister(getDLXRegisterNumber(instr->reg(1)), 1);
			Register dst = getRegisterForTargetDLXRegister(destination);
			writeMul(AL, false, dst, src1, src2);
			storeTargetDLXRegister(destination);
		}
	}
	...
	else if (line.textInstruction->opcode() == "STW")
	{
		...
		if (destination > 0)
		{
			...
			Register src = loadDLXRegister(getDLXRegisterNumber(instr->reg(1)), 1);
			writeMov(AL, true, SECOND_ARGUMENT_CACHE_REGISTER, src);
			writeRev(AL, SECOND_ARGUMENT_CACHE_REGISTER, SECOND_ARGUMENT_CACHE_REGISTER);
			
			writeSTR(AL, OFFSET, true, SECOND_ARGUMENT_CACHE_REGISTER, dataMemoryOffsetRegister, instr->baseAddress());
		}
	}
