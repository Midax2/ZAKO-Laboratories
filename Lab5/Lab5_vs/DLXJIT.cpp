...
void DLXJIT::loadCode(istream& codStream)
{
	...
	if (line.find("[Code Memory Content]") == string::npos)
...
void DLXJIT::loadData(std::istream& datStream)
{
	...
	if (line.find("[Data Memory]") == string::npos)
	...
	bool foundContentHeader = false;
	while (getline(datStream, line))
	{
		if (line.find("[Data Memory Content]") != string::npos)
		{
			foundContentHeader = true;
			break;
		}
		...
		if (trimmedLine.compare(0, sizeKey.size(), sizeKey) == 0)
		{
			size = (DLXDatRepresentationSize)stol(trimmedLine.substr(sizeKey.size()));
		}
		else if (trimmedLine.compare(0, baseKey.size(), baseKey) == 0)
		{
			base = (DLXDatRepresentationFormatBase)stol(trimmedLine.substr(baseKey.size()));
		}
	}
